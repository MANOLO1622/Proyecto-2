$(document).ready(function () {
    ...
    $.get(siteVariables.webAPIUrl + "getUsers",
        function (data) {
            users = data.Data;
            console.log(users);
        });
            ...
            if (userFound) {
                ...
                switch (loggedUser.Rol) {
                    case '1':
                        ...
                        break;
                    case '2':
                        window.location.href = siteVariables.webAppUrl + "dashboard/airport/" + loggedUser.AssignedID;
                        //add hiding clases to corresponding items in the nav bar
                        break;
                    case '3':
                        window.location.href = siteVariables.webAppUrl + "dashboard/airline/" + loggedUser.AssignedID;
                        break;
                    case '4':
                        window.location.href = siteVariables.webAppUrl + "dashboard/user/" + loggedUser.ID;
                        break;
                }
            } else {
                swal({
